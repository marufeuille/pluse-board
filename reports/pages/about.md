...
ACWR は心拍ベースの AZM を負荷にしているため、心拍が上がりきらない筋トレでは
負荷がほとんど立たず実態を映さない。そこで筋トレは頻度・時間・連続達成で別軸に追う。
集計対象は `exerciseType = 'STRENGTH_TRAINING'` のみ。

| 指標 | 定義 |
|---|---|
| **セッション数** | 当該期間の筋トレログ件数 |
| **頻度（active_days）** | 週内に筋トレした日数 |
| **筋トレ時間** | `endTime - startTime`（分）の合計 |
| **カロリー** | `metricsSummary.caloriesKcal` の合計（Fitbit 推定値） |
| **達成週** | 週のセッション数が **3 回以上** の週 |
| **ストリーク** | 達成週が連続している数。最長を「ベストストリーク」とする |

### 実装上の注意

- **欠週は 0 埋め必須**: 筋トレ 0 回の週を calendar JOIN で埋めないと、サボった週が消えて
  連続達成（ストリーク）が途切れなくなる（ACWR の休養日 0 埋めと同じ思想）。
- **最新週は進行中**: 最新週がまだ週 3 回に達していない場合、現在のストリークは 0 表示になる。
- 週はじまりは下記のとおり日曜。

実装: [`sqlmesh_project/models/marts/mart_strength_daily.sql`](https://github.com/marufeuille/pluse-board/blob/main/sqlmesh_project/models/marts/mart_strength_daily.sql) ・ [`mart_strength_weekly.sql`](https://github.com/marufeuille/pluse-board/blob/main/sqlmesh_project/models/marts/mart_strength_weekly.sql) ・ [`mart_strength_streak.sql`](https://github.com/marufeuille/pluse-board/blob/main/sqlmesh_project/models/marts/mart_strength_streak.sql)
